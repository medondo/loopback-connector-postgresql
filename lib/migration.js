...
const defaultExecutionOptions = {forceMaster: true};
...
function mixinMigration(PostgreSQL) {
  ...
  PostgreSQL.prototype.showFields = function(model, cb) {
    ...
    this.schema(model) + '\'';
    this.execute(sql, [], defaultExecutionOptions, function(err, fields) {
      if (err) {
        return cb(err);
      } else {
  ...
  PostgreSQL.prototype.showIndexes = function(model, cb) {
    const sql = 'SELECT t.relname AS "table", i.relname AS "name", ' +
          ...
          this.schema(model) + '\')';
    this.execute(sql, [], defaultExecutionOptions, function(err, indexes) {
      if (err) {
        return cb(err);
      } else {
        cb(err, indexes);
