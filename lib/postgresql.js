...
const debugReplica = require('debug')('loopback:connector:postgresql:replica');
const Promise = require('bluebird');
const wrr = require('@lukeed/wrr');

const PostgresReplications = function(writer, readers) {
  const end = writer.end.bind(writer);
  const write = writer.query.bind(writer);
  const replicas = [].concat(readers || writer);
  const toReplica = replicas.length === 1 ? (() => replicas[0]) : wrr(
    replicas.map(x => ({item: x.item || x, weight: x.weight || 1})),
  );

  writer.query = function(text) {
    let func = write;
    let client = writer;

    const forceMaster = arguments['3'] || isDbSchemaQuery(text);
    const forceReplica = arguments['4'];

    if (forceReplica || !forceMaster && isReadOnlyQuery(text)) {
      debugReplica('use replica for %s', text);
      client = toReplica();
      if (client !== writer) {
        func = client.query;
      }
    ...
    return func.apply(client, arguments);
  };

  writer.end = function() {
    debugReplica('replica master end');
    end(); // the original
    if (replicas.length > 0) {
      
      replicas.forEach((r, index )=> {
        debugReplica(`replica ${index} end`);
        r.end();
      });
    }
  };
  ...
  function isReadOnlyQuery(sql) {
    const forbiddenKeywords = ['FOR UPDATE', 'FOR SHARE'];
  ...
  function isDbSchemaQuery(sql) {
    const internalKeywords = ['information_schema', 'table_schema'];
...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  const replications = dbSettings.replications;
  delete dbSettings.replications;

  dataSource.connector = new PostgreSQL(postgresql, dbSettings, replications);
 ...
 * @constructor
 */
function PostgreSQL(postgresql, settings, replications = null) {
  // this.name = 'postgresql';
  // this._models = {};
  // this.settings = settings;
  const config = {settings, ...{replications: replications}};

  this.constructor.super_.call(this, 'postgresql', config);
  ...
  if (replications) {
    const masterConfig = replications.master;
    const slavesConfig = replications.slaves;

    const master = new postgresql.Pool(masterConfig);
    const slaves = slavesConfig ? slavesConfig.map(config => new postgresql.Pool(config)) : [];

    this.pg = PostgresReplications(master, slaves);
  } else {
    this.pg = new postgresql.Pool(settings);
  }
...
PostgreSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  const forceReplica = options.forceReplica || false;
  function executeWithConnection(connection, releaseCb) {
    ...
    }, options.forceMaster || false, forceReplica);
  }

  const transaction = options.transaction;
  if (transaction && transaction.connector === this && !forceReplica) {
